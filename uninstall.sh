...
if [[ -f "$CLAUDE_SETTINGS" ]]; then
    ...
    if grep -q "claude-code-notifier" "$CLAUDE_SETTINGS" 2>/dev/null; then
        ...
        echo "- SessionStart hook pointing to $INSTALL_DIR/session_start_hook.sh"
        echo "- Stop hook pointing to $INSTALL_DIR/stop_hook.sh"
        echo "- Notification hook pointing to $INSTALL_DIR/stop_hook.sh"
        echo "- SessionEnd hook pointing to $INSTALL_DIR/session_end_hook.sh"
        echo ""
