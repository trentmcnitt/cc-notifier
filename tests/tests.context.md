...
- `test_notify_continues_to_push_when_local_fails` - Notify gracefully handles local notification failure and continues to push - Validates graceful degradation when local notifications fail
...
### TestCoreWorkflows (8 tests) - End-to-End Workflow Validation
...
- `test_init_workflow_without_hammerspoon` - Init falls back to UNAVAILABLE when Hammerspoon is missing - Validates graceful degradation without Hammerspoon
- `test_notify_sends_unconditionally_without_hammerspoon` - Notify sends local notification without window comparison when UNAVAILABLE - Validates unconditional notification without window tracking
